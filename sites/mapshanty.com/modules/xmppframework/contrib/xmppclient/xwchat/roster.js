...
function xcSetRosterClick() {
  ...
  JQ('div.xcContact').unbind().mousedown(function(e) {
    ...
    if (buttoncode == 1) {
      var w = xcMsgUser(id); // start a new chat for the user
    } else {
    ...
    if (contact.getSubscription() == 'both') {
      ...
      JQ('li#resubscribe').show().html('Subscribe');
    } else {
      JQ('li#resubscribe').show().html('Resubscribe');
    };
    ...
    JQ('.xcContextMenu > ul > li').unbind().click(function() {
      if (this.id == 'archive') {
        ...
        var w = xcMsgUser(id); // open a new chat window to chat one on one with the user
