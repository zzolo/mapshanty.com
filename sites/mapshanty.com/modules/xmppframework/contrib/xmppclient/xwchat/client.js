...
function xcSetMsg(msg, error) {
  msg = '<div>' + msg + '</div>';
  if (error) {
    JQ('#msg').show().addClass('xcError').append(msg);
  } else {
    JQ('#msg').show().removeClass('xcError').append(msg);
  };
  if (timer) { clearTimeout(timer); timer = 0; };
  timer = setTimeout(function() { JQ('#msg').hide().removeClass('xcError').html(''); timer = 0; }, XC.ERRORTIMEOUT);
};
...
function xcDisconnected() {
  ...
  JQ('#xcCL').html(''); // removing the contacts so the person does not think they are still online
};
...
function xcConnected() {
  try {
    JQ('#xcCL').html(''); // incase it is a re-connect then make sure we clear all information from the roster
    ...
    setTimeout(function() {
      ...
      if (JQ(document).getUrlParam('locale')) { XC.locale = JQ(document).getUrlParam('locale'); };
  ...
  switch (show) {
    ...
    default:
      try {
        ...
        if (!con.connected() && status != 'Unavailable') {
          xcLogin();
          return;
        };
